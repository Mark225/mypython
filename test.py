...
class MainWindow:
    def __init__(self):
        ...
        self.label_name = Label(self.frame,text = "请输入单个汉字:")
        self.label_age = Label(self.frame,text = "请输入Excel表路径:")

        self.label_multi_var = StringVar()
        self.text_name = Entry(self.frame)
        self.text_age = Entry(self.frame,textvariable=self.label_multi_var)

        self.label_name.grid(row = 0,column = 0)
        self.label_age.grid(row = 1,column = 0)

        self.button_ok = Button(self.frame,text = "ok",command = start,width = 10)
